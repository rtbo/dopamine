...
int luaPrivPkgConfArgvSplit(lua_State* L) nothrow
{
    ...
    debug { import std.stdio : writefln; try { writefln!"will split %s"(fromStringz(str)); } catch (Exception) {} }
    ...
    {
        debug { import std.stdio : writefln; try { writefln!"%s/%s"(idx+1, argc); } catch (Exception) {} }
        debug { import std.stdio : writefln; try { writefln!"will push %s"(fromStringz(argv[idx])); } catch (Exception) {} }
